...
const fitsIn = (inner: Size, outer: Size) => {
	return outer.width >= inner.width && outer.height >= inner.height;
};
const isSameSize = (self: Size, other: Size) => {
...
class AvailableSpace {
	...
	insertRect(rect: Size): AvailableSpace | null {
		...
		var widthDiff = this.#rect.width - rect.width;
		var heightDiff = this.#rect.height - rect.height;
...
export class DynamicSpriteSheet {
	...
	static #maxCachableArea = Math.pow(this.#baseTextureSize, 2) * 0.5;
